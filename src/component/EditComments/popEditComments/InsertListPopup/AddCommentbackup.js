...
const AddComment = ({ onClose, value, setfetch }) => {
  ...
  useEffect(() => {
    if (text) {
      ...
      const shortListedText = shortListed;
      let newText = text;
      for (const key in shortListedText) {
        const Textkey = shortListedText[key];
        // console.log("Textkey", Textkey);

        if (newText.includes(Textkey.text)) {
          newText = newText.replace(Textkey.text, (match) => {
            let styledMatch = match;
            if (Textkey.bold) styledMatch = `<b>${styledMatch}</b>`;
            if (Textkey.italic) styledMatch = `<i>${styledMatch}</i>`;
            if (Textkey.underline) styledMatch = `<u>${styledMatch}</u>`;
            styledMatch = `<span style="color: ${Textkey.color};">${styledMatch}</span>`;
            // console.log("stledmatch", styledMatch);
            return styledMatch;
          });
        }
      }
  ...
  }, [shortListed, isBold, isItalic, isUnderline, highlightColor, text]);
  // useEffect(() => {
  //   const dsfdsj = localStorage.getItem("styledText");
  //   console.log(dsfdsj);
  // });
